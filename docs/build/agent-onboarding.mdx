...
The same zero-auth self-service flow works via the CLI — ideal for autonomous agents with shell access (Claude Code, Codex, Devin, etc.). The CLI auto-provisions on first use — no manual setup:

### Just Run It

The first command auto-provisions an anonymous tenant and saves credentials to `~/.headlessly/config.json`:

```bash
# First run — auto-provisions, then returns results
...
# Create entities
headlessly do create Contact --data '{"name":"Alice Smith","email":"alice@example.com","stage":"Lead"}' --json

# Search
headlessly search Contact --json --fields name,email,stage
...
# Query CDC events
headlessly fetch events --json --limit 10 --fields id,type,event,ts

# Preview before executing
headlessly do create Deal --data '{"name":"Big Deal","value":50000}' --dry-run --json
```

No curl. No login. No config. The agent just runs commands and it works. All commands support `--json` for structured output, `--fields` for context window discipline, and `--dry-run` for safe previews.

### Claim

Commit `.github/workflows/headlessly.yml` to link the tenant to a GitHub identity:
...
The push event verifies the commit author. Data migrates from sandbox to persistent storage. No OAuth, no browser, no signup form.

### MCP vs CLI

Both interfaces support the full self-service flow:

| Capability | MCP | CLI |
|-----------|-----|-----|
| Zero-auth start | L0 automatic | Auto-provisions on first command |
| Read entities | `search` / `fetch` tools | `search` / `fetch` commands |
| Write entities | `do` tool | `do create` / `do update` / `do delete` |
| Preview mutations | `try` tool | `--dry-run` flag |
| Query events | `fetch events` | `fetch events --json` |
| Field masking | Response filtering | `--fields` flag |
| Structured output | Always JSON | `--json` flag |
| Claim tenant | `$.session.claim()` | Commit workflow file |

MCP is for agents that speak the Model Context Protocol (Claude, GPT, etc.). CLI is for agents with shell access (Claude Code, Codex, Devin, terminal-based agents). Same data, same verbs, same tenant.
