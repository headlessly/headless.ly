...
The same zero-auth self-service flow works via the CLI — ideal for autonomous agents with shell access (Claude Code, Codex, Devin, etc.):

### 1. Provision

```bash
# Provision an anonymous tenant (no auth required)
curl -sX POST https://api.headless.ly/provision | jq .
```

Returns a `tenantId`, `sessionToken`, and `claimToken`:

```json
{
  "tenantId": "anon_abc123",
  "sessionToken": "ses_xyz789",
  "claimToken": "clm_def456",
  "level": 1,
...
```bash
headlessly login --tenant anon_abc123 --api-key ses_xyz789 \
  --endpoint https://api.headless.ly/~anon_abc123
```
...
All commands support `--json` for structured output, `--fields` for context window discipline, and `--dry-run` for safe previews.
...
| Capability | MCP | CLI |
|-----------|-----|-----|
| Zero-auth start | L0 automatic | `curl /provision` + `login` |
