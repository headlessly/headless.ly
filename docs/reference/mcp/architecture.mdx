...
headless.ly's MCP server exposes exactly three tools: `search`, `fetch`, and `do`. They cover CRM, project management, billing, content, support, analytics, marketing, experimentation, and workflows. All 32 entity types. All CRUD operations plus custom domain verbs. Across 52 composable systems.
...
{ "type": "Contact", "filter": { "stage": "Lead", "company": { "$exists": true } }, "limit": 10 }
```
...
```json title="headless.ly/mcp#search"
{ "type": "Contact", "filter": { "stage": "Lead", "company": { "$exists": true } }, "limit": 5 }
```

**Step 2: Fetch the company details for context**

```json title="headless.ly/mcp#fetch"
{ "type": "Company", "id": "company_e5JhLzXc" }
```
