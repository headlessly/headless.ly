...
| Projects | `$.Project`, `$.Task`, `$.Milestone`, `$.Comment` |
| Content | `$.Page`, `$.Post`, `$.Asset`, `$.Collection` |
| Billing | `$.Product`, `$.Plan`, `$.Subscription`, `$.Invoice`, `$.Payment` |
| Support | `$.Ticket`, `$.Article`, `$.Reply` |
...
```ts title="headless.ly/mcp#do"
await $.Task.assign('task_qW8eYuHn', { assignee: 'member_dQz8FhLm' })
```
...
const contacts = await $.Contact.find({
  ...
  'company.industry': segment.industry
})
